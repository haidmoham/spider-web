...
let stridePayload, stridePolicy, strideVisual, strideControlStep = 0;
let mujoco;
let model;
let data;
let bodyAccessors;
...
function applyGaitControl() {
  if (data.time + 1e-10 >= strideControlStep * 0.02) {
    data.ctrl.set(stridePolicy.targets(data)); strideControlStep += 1;
  }
...
function setStandingPose() {
  stridePolicy.reset(); strideControlStep = 0;
...
function updateRobotVisual() {
  robotVisual.torso.visible = false;
  robotVisual.legs.forEach(leg => Object.values(leg).forEach(mesh => { mesh.visible = false; }));
...
function render() {
  const [roll, pitch] = bodyErrors();
  const state = {contacts: contactStates(), roll, pitch};
...
async function initialise() {
  try {
    ...
    const [loadedMujoco, modelXml, manifest] = await Promise.all([
      loadMujoco(),
      fetch('./stride/model.xml').then((response) => {
    ...
    stridePayload = await fetch('./stride/walk_fast_500.json').then(r => r.json());
    stridePolicy = new CadencePolicy(stridePayload);
    strideVisual = new StrideVisual(scene, stridePayload);
    mujoco = loadedMujoco;
    ...
    restart();
    status.textContent = `Live 3D · C-1N // 03 � STRIDE · MuJoCo ${mujoco.mj_versionString()} · walk_fast_500`;
